...
/**
  * This is an event sourced entity. It has a state, [[ExchangeState]], which
  ...
  */
class ExchangeEntity extends PersistentEntity {

  override type Command = ExchangeCommand[_]
  override type Event = ExchangeEvent
  override type State = ExchangeState

  /**
    * The initial state. This is used if there is no snapshotted state to be found.
    */
  override def initialState: ExchangeState = ExchangeState("Hello", LocalDateTime.now.toString)
  ...
  override def behavior: Behavior = {
    case ExchangeState(message, _) => Actions().onCommand[UseGreetingMessage, Done] {
      ...
      case (GreetingMessageChanged(newMessage), state) =>
        ...
        // the event.
        ExchangeState(newMessage, LocalDateTime.now().toString)
...
/**
  ...
  */
case class ExchangeState(message: String, timestamp: String)
...
object ExchangeState {
  /**
    ...
    */
  implicit val format: Format[ExchangeState] = Json.format
}

/**
  * This interface defines all the events that the ExchangeEntity supports.
  */
sealed trait ExchangeEvent extends AggregateEvent[ExchangeEvent] {
  def aggregateTag = ExchangeEvent.Tag
}

object ExchangeEvent {
  val Tag = AggregateEventTag[ExchangeEvent]
}

/**
  * An event that represents a change in greeting message.
  */
case class GreetingMessageChanged(message: String) extends ExchangeEvent
...
/**
  ...
  */
sealed trait ExchangeCommand[R] extends ReplyType[R]
...
/**
  ...
  */
case class UseGreetingMessage(message: String) extends ExchangeCommand[Done]
...
/**
  ...
  * person.
  */
case class Hello(name: String) extends ExchangeCommand[String]
...
/**
  ...
  * application loader.
  */
object ExchangeSerializerRegistry extends JsonSerializerRegistry {
  override def serializers: Seq[JsonSerializer[_]] = Seq(
    JsonSerializer[UseGreetingMessage],
    JsonSerializer[Hello],
    JsonSerializer[GreetingMessageChanged],
    JsonSerializer[ExchangeState]
  )
}
